 ...
 * multi-thousand-icon package for five entries. Sources, per entry:
 *
...
export interface AgentIcon {
  ...
  title: string;
  /**
   * The brand's color, no leading `#`. Genuinely black for cursor/copilot/
   * grok (their real marks) — worth knowing since that renders near-invisible
   * in "color" mode against Silo's dark theme; codex has no single official
   * hex (OpenAI's mark is black/white too), so this uses a representative
   * blue from `@lobehub`'s own colored treatment of it instead, chosen partly
   * because it's actually visible on a dark background.
   */
  hex: string;
