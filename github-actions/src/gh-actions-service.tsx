...
export class GhActionsService {
  ...
  getBadgesFor(workspaceId: string): WorkspaceBadge[] {
    ...
    for (const s of states) {
      failed += aggregateRunState(s.runs, clearedBefore).failed;
    }
    if (failed === 0) return [];
  ...
  getDecorationsFor(workspaceId: string): WorkspaceStatusRow[] {
    ...
    for (const s of states) {
      ...
      failed += agg.failed;
      running += agg.running;
      const top = selectFailedRuns(s.runs, clearedBefore)[0];
  ...
  getTooltipContent(): string {
    ...
    return getRichTooltip(state, primary, ghStore.getClearedAt(activeId));
  }
