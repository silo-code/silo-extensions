...
import type { AuthState, GitHubApiError, IssueDetail, IssueListItem } from "./github-issue-api";
import { DEFAULT_FILTER, type IssueFilter } from "./filters";
...
export type WorkspaceIssueError =
  | { kind: "unauthenticated" }
  | { kind: "api-error"; error: GitHubApiError }
  | { kind: "no-repo" };

export interface WorkspaceIssueState {
  ...
  error: WorkspaceIssueError | null;
}
...
export class IssueStore {
  ...
  private _repoStates = new Map<string, WorkspaceIssueState>();
  ...
  private _workspaceFilter = new Map<string, IssueFilter>();
  ...
  getRepoStates(workspaceId: string): WorkspaceIssueState[] {
    const prefix = `${workspaceId}:`;
    const result: WorkspaceIssueState[] = [];
  ...
  hydrate(storage: ExtensionStorage): void {
    ...
    const savedFilter = storage.get<Record<string, IssueFilter>>("workspaceFilter") ?? {};
  ...
  setRepoState(workspaceId: string, owner: string, repo: string, state: WorkspaceIssueState): void {
  ...
  setDetail(repoKey: string, number: number, detail: IssueDetail): void {
  ...
  // Defaults true (opt-out) — monitoring every detected repo is the expected
  // behavior until a user opts out.
  ...
  getWorkspaceFilter(workspaceId: string): IssueFilter {
    return this._workspaceFilter.get(workspaceId) ?? DEFAULT_FILTER;
  }

  setWorkspaceFilter(workspaceId: string, value: IssueFilter): void {
    this._workspaceFilter.set(workspaceId, value);
    const record: Record<string, IssueFilter> = {};
