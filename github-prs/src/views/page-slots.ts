...
// The stack is linear (list → detail → {commits → commit, files}) with one
// exception: "commits" and "files" are *siblings*, both reached directly
// from detail, so they share depth 2. "Current" is decided by exact kind
// match (not depth equality) precisely because of that — with depth alone,
// viewing "commits" would also compute "current" for "files" (same depth),
// putting both pages at translateX(0) simultaneously. Depth only decides
// which side a non-current page parks on.
